"""
Description : Runs hyperparameter search for two-stage aggregate kernel ridge regression experiment

Usage: run_hyperparams_search_two_stage_kernel_ridge_regression.py  [options] --cfg=<path_to_config> --o=<output_dir>
...
from run_two_stage_kernel_ridge_regression import migrate_to_device, fit, predict
...
def make_model(cfg, data, hyperparams):
    ...
    kernel_2d.lengthscale = hyperparams['lengthscale_2d'] * torch.ones(ard_num_dims)
    ...
    kernel_3d = RFFKernel(nu=cfg['model']['nu_3d'],
                          ...
                          ard_num_dims=ard_num_dims)
    kernel_3d.lengthscale = hyperparams['lengthscale_3d'] * torch.ones(ard_num_dims)

...
def make_3d_toy_data(cfg):
    ...
    toy_data = ToyData(dataset=dataset, standard_dataset=standard_dataset,
                       x_by_column_std=x_by_column_std, x_std=x_std,
                       z_grid=z_grid, z_std=z_std,
                       gt_grid=gt_grid,
                       h=h, h_std=h_std)
    return toy_data


def make_2d_and_3d_toy_data(cfg):
    # Load dataset as defined in main code
    dataset = preproc.load_dataset(cfg['dataset']['path'])
    ...
    # Encapsulate into named tuple
    ToyData = namedtuple('ToyData', ['dataset', 'standard_dataset', 'x_by_column_std', 'x_std', 'y_grid_std', 'y_std', 'z_grid', 'z_std', 'gt_grid', 'h', 'h_std'])
    toy_data = ToyData(dataset=dataset, standard_dataset=standard_dataset,
                       x_by_column_std=x_by_column_std, x_std=x_std,
                       y_grid_std=y_grid_std, y_std=y_std,
                       z_grid=z_grid, z_std=z_std,
                       gt_grid=gt_grid,
                       h=h, h_std=h_std)
    return toy_data

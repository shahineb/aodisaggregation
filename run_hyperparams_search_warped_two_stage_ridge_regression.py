"""
Description : Runs hyperparameter search for warped two-stage aggregate ridge regression experiment

Usage: run_hyperparams_search_warped_two_stage_ridge_regression.py  [options] --cfg=<path_to_config> --o=<output_dir>
...
from run_warped_two_stage_ridge_regression import migrate_to_device, fit, predict
...
def make_model(cfg, data, hyperparams):
    ...
    # Instantiate model
    model = WarpedTwoStageAggregateRidgeRegression(lbda_2d=hyperparams['lbda_2d'],
                                                   lbda_3d=hyperparams['lbda_3d'],
                                                   transform=transform,
                                                   aggregate_fn=trpz,
                                                   ndim=len(cfg['dataset']['3d_covariates']) + 4,
                                                   fit_intercept_2d=hyperparams['fit_intercept_2d'],
                                                   fit_intercept_3d=hyperparams['fit_intercept_3d'])

"""
Description : Runs hyperparameter search for warped two-stage aggregate kernel ridge regression experiment

Usage: run_hyperparams_search_warped_two_stage_kernel_ridge_regression.py  [options] --cfg=<path_to_config> --o=<output_dir>
...
from run_warped_two_stage_kernel_ridge_regression import migrate_to_device, fit, predict
...
def main(args, cfg):
    ...
    for j, hyperparams in enumerate(hyperparams_grid):
        ...
        for i, (train_idx, test_idx) in enumerate(kfold.split(data.x_by_column_std)):
            ...
            model = fit(cfg=cfg, model=model, data=train_data)
...
def make_model(cfg, data, hyperparams):
    ...
    kernel_2d.lengthscale = hyperparams['lengthscale_2d'] * torch.ones(ard_num_dims_2d)
    ...
    kernel_3d.lengthscale = hyperparams['lengthscale_3d'] * torch.ones(ard_num_dims_3d)
    ...
    model = WarpedTwoStageAggregateKernelRidgeRegression(kernel_2d=kernel_2d.to(device),
                                                         ...
                                                         lbda_2d=hyperparams['lbda_2d'],
                                                         lbda_3d=hyperparams['lbda_3d'],
                                                         transform=transform,
                                                         aggregate_fn=trpz)
